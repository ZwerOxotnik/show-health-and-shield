...
local function on_tick()
	for _, player in pairs(game.connected_players) do
		...
		if character and character.health ~= nil and not character.vehicle then
			variants[settings.get_player_settings(player)["shas_player_hp_mode"].value].show_hp(character, player)
			for _, target in pairs(player.force.connected_players) do
				variants[settings.get_player_settings(target)["shas_player_shield_mode"].value].show_shield(character, target)
			end
		end
	end
	...
	for index, vehicle in pairs(data.vehicles_shield) do
		if vehicle.entity.valid then
			for _, player in pairs(vehicle.entity.force.connected_players) do
				variants[settings.get_player_settings(player)["shas_vehicle_shield_mode"].value].show_shield_for_vehicles(vehicle, player)
			end
		else
